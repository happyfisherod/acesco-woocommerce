	...
	function urna_tbay_breadcrumbs() {
		...
		if (!is_home() && !is_front_page() || is_paged()) {
			...
			if (is_category()) {
				...
				echo trim($before) .get_the_title($post->ID) . $after;
				...
				echo trim($before) . get_the_title($page->ID) . $after;
	...
	function urna_get_display_footer_builder($footer) {
		...
		foreach ( $posts as $post ) {
			if( urna_is_elementor_activated() && Elementor\Plugin::$instance->db->is_built_with_elementor( $post->ID ) ) {
				...
				$ouput .= Elementor\Plugin::instance()->frontend->get_builder_content_for_display( $post->ID );
				$ouput .= '</footer>';
