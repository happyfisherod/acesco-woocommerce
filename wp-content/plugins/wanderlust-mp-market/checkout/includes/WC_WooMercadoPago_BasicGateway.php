...
class WC_WooMercadoPago_BasicGateway extends WC_Payment_Gateway {
	...
	public function init_form_fields() {
      ...
      'marketplace_comission' => array(
				'title' => __( 'Commission charge', 'woocommerce-mercadopago' ),
				'type' => 'number',
				'description' => '% of total order.',
				'default' => '1'
			),
		);
	...
	public function custom_process_admin_options() {
		...
		foreach ( $this->get_form_fields() as $key => $field ) {
			if ( 'title' !== $this->get_field_type( $field ) ) {
				...
				} elseif ( $key == 'marketplace_comission' ) {
	...
	public function render_order_form( $order_id ) {
		...
		if ( 'modal' == $this->method && $url ) {
			...
			$html .= '<p></p><p>' . wordwrap(
						__( 'Gracias por su compra. Prosiga con su pago completando la información abajo.', 'woocommerce-mercadopago' ),
						60, '<br>'
					) . '</p>
					...
					'</a> <a class="button cancel" href="' . esc_url( $order->get_cancel_order_url() ) . '">' .
						__( 'Cancelar orden', 'woocommerce-mercadopago' ) .
					'</a>';
			return $html;
			// ===== The checkout is made by displaying a modal to the customer =====
      ...
      //mail('wanderlustwebdesign@gmail.com', 'ave 719 WooMercadoPago_BasicGateway', print_r($url, true));
    ...
    if(is_array($shipping_data)):
      foreach($shipping_data as $k=>$sm){
        if($sm['total'] > 1){
          $costo_envio = str_replace(".",",",$sm['total']);
        }    
      }
     endif; 
    $total_without_shipping = $total_order - $costo_envio;
    $comision = $total_without_shipping * $this->settings['marketplace_comission'] / 100;
    $comision = $comision ;
      ...
      'additional_info' => $order->id,
		...
		if ( ! strrpos( get_site_url(), 'localhost' ) ) {
			...
			if ( empty( $notification_url ) || filter_var( $notification_url, FILTER_VALIDATE_URL ) === FALSE ) {
				$preferences['notification_url'] = 'https://construyamos.com/colombia/?wc-api=wanderlust_meli_mp';
			} else {
				$preferences['notification_url'] = 'https://construyamos.com/colombia/?wc-api=wanderlust_meli_mp';
    ...
    if ( sizeof( $order->get_items() ) > 0 ) {
      foreach ( $order->get_items() as $item ) {
        if ( $item['qty'] ) {
          ...
          $mpdata = get_user_meta( $post_author_id, 'mercado_pago_response', true);
      ...
      $request = array(
         ...
         'params' => array(
           'access_token' => $mpdata->access_token//$this->get_access_token()
        ),
         'headers' => array(
	    ...
	    $checkout_info = MPRestClient::post( $request, $this->version );
      ...
      $deb = json_encode( $ex, JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE );
      mail('wanderlustwebdesign@gmail.com', 'WC_WooMercadoPago_BasicGateway 1063', print_r($deb, true));
