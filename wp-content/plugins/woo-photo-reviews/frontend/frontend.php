...
class VI_WOO_PHOTO_REVIEWS_Frontend_Frontend {
	...
	public function frontend_enqueue() {
		...
		wp_localize_script( 'woocommerce-photo-reviews-script', 'woocommerce_photo_reviews_params', array(
				...
				'warning_max_files'          => sprintf( _n( 'You can only upload maximum of %s file', 'You can only upload maximum of %s files', $this->settings->get_params( 'photo', 'maxfiles' ), 'woo-photo-reviews' ), $this->settings->get_params( 'photo', 'maxfiles' ) ),
		...
		if ( 'on' == $this->settings->get_params( 'photo', 'enable' ) ) {
			if ( $this->settings->get_params( 'photo', 'display' ) == 1 ) {
				...
				wp_add_inline_style( 'wcpr-masonry-style', $css_inline2 );
				...
				$css_inline2 = ".reviews-images-item{margin-right: 2px;padding: 0;float:left;border-radius: 3px;}.kt-reviews-image-container .kt-wc-reviews-images-wrap-wrap .reviews-images-item .review-images{float: left !important;height: 48px !important;width:auto !important;border-radius: 3px;}";
	...
	public function stars_count( $star, $post_id ) {
		$agrs   = array(
			'post_id'    => $post_id,
			'count'      => true,
			'status'     => 'approve',
	...
	public function photo_reviews( $r ) {
		...
		?>
        <div class="wcpr-grid <?php echo 'wcpr-masonry-' . $this->settings->get_params( 'photo', 'col_num' ) . '-col'; ?>">
			<?php
			foreach ( $my_comments as $v ) {
				if ( $v->comment_parent ) {
					continue;
				}
				$comment_children = $v->get_children();
                    ...
                    <div class="wcpr-content">
						...
						if ( is_array( $img_post_ids ) && count( $img_post_ids ) > 0 ) {
							...
							if ( count( $img_post_ids ) > 1 ) {
								foreach ( $img_post_ids as $img_post_ids_k => $img_post_id ) {
									if ( ! wc_is_valid_url( $img_post_id ) ) {
										...
										echo '<div class="reviews-images-wrap"><a data-image_index="' . $img_post_ids_k . '" data-image_src="' . $data_image_src . '" data-image_caption="" rel="nofollow" href="' . $href . '"><img class="reviews-images" src="' . $thumb . '" alt="' . $image_alt . '"/></a></div>';
									} else {
										echo '<div class="reviews-images-wrap"><a data-image_index="' . $img_post_ids_k . '" rel="nofollow" href="' . $img_post_id . '"><img class="reviews-images" src="' . $img_post_id . '" alt="' . $product_title . '"/></a></div>';
									}
								}
							}
							echo '</div>';
							...
							if ( ! wc_is_valid_url( $first_ele ) ) {
								...
								$src               = ( isset( $image_data['sizes']['wcpr-photo-reviews'] ) ? wp_get_attachment_image_url( $first_ele, 'wcpr-photo-reviews' ) : ( isset( $image_data['sizes']['medium_large'] ) ? wp_get_attachment_image_url( $first_ele, 'medium_large' ) : ( isset( $image_data['sizes']['medium'] ) ? wp_get_attachment_image_url( $first_ele, 'medium' ) : $data_original_src ) ) );
								echo '<div class="reviews-images-wrap-right"><img class="reviews-images" data-original_src="' . $data_original_src . '" src="' . apply_filters( 'woocommerce_photo_reviews_masonry_thumbnail_main', $src, $first_ele ) . '" alt="' . $image_alt . '"/></div>';
							} else {
                        ...
                        <div class="review-content-container">
							...
							if ( '0' === $v->comment_approved ) {
                                ...
                                <div class="review-content-container-top">
                                    ...
                                    <div class="wcpr-review-rating">
										...
										if ( $this->settings->get_params( 'photo', 'show_review_date' ) ) {
											?>
                                            <div class="wcpr-review-date">
												...
												comment_date( $review_date_format, $v );
												?>
                                            </div>
											<?php
										}
							...
							?>
                            <div class="wcpr-review-content"><?php echo ucfirst( $v->comment_content ); ?></div>
							<?php
										...
										foreach ( $comment_children as $comment_child ) {
                                            ...
                                            <div class="wcpr-comment-child">
                                                ...
                                                <div class="wcpr-comment-child-content">
													<?php echo ucfirst( $comment_child->comment_content ); ?>
                                                </div>
                                            </div>
											<?php
										}
